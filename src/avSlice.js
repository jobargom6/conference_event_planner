...
export const avSlice = createSlice({
  ...
  reducers: {
    incrementAvQuantity: (state, action) => {
      const item = state[action.payload]; //action.payload contiene el identificador del artículo a incrementar.
      if(item){
        item.quantity++;
      }                                    // El reductor recupera el artículo del estado usando state[action.payload]   
    },
